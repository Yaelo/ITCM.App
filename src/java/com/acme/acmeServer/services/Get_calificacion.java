 ...
 * @author Yaelo
 */
@Path("get_calif/{ncontrol}")
public class Get_calificacion {

    @Context
    private UriInfo context;

    /**
     * Creates a new instance of Get_calificacion
     */
    public Get_calificacion() {
    }

    /**
     * Retrieves representation of an instance of
     * com.acme.acmeServer.services.Get_calificacion
     *
...
//                        + "(SELECT user from usuario where user = '" + userID + "' AND pass = " + pass + ")");
            cs = connection.prepareCall("{call get_calificaciones(?)}");
            cs.setString(1, ncontrol);
            rs = cs.executeQuery();
            ArrayList<JsonObject> myList = new ArrayList();
            while (rs.next()) {
                objetoJson = Json.createObjectBuilder()
                        .add("materia", rs.getString("Materia"))
                        .add("calificacion", rs.getString("Calificacion"))
                        .build();
    ...
    /**
     * PUT method for updating or creating an instance of Get_calificacion
     *
